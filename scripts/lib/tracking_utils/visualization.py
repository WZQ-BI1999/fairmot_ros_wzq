...
def plot_tracking(image, tlwhs, obj_ids, scores=None, frame_id=0, fps=0., dists=0, ids2=None):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        obj_dist = float(dists[i])
        id_text = '{}'.format(int(obj_id))
        dist_text = '{:.2f}%'.format(float((1-obj_dist)*100))
        if ids2 is not None:
        ...
        cv2.putText(im, dist_text, (intbox[0], intbox[1] + 60), cv2.FONT_HERSHEY_PLAIN, text_scale, (0, 0, 255),
                    thickness=text_thickness)
